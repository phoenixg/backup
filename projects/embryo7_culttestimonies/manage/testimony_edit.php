...
<div class="main">
	...
	/*
	 * ***分页**************
	 * 每一页显示记录数：$pageSize(Default 20)
	 * 总记录数：$totalRecords
	 * 总页数：$totalPageNum = f($totalRecords, $pageSize)
	 * */
	...
	//get $totalRecords
	$sql = "SELECT count(*) as `totalRecords` FROM `culttestimonies_testimonies`;";
	$pds = $pdo->query($sql);
	$row = $pds->fetch(PDO::FETCH_ASSOC);
	$totalRecords = $row['totalRecords'];
	
	//calculate out $totalPageNum
	$totalPageNum = ceil($totalRecords/$pageSize);
		
	//get the $currentPageId
	$currentPageId = $_GET['pid']==0?1:$_GET['pid'];
	
	//show record list according to the limited $pageSize 
	...
	while ($row = $pds->fetch(PDO::FETCH_ASSOC)){
		...
		echo '<span>'.$row['testimony_date'].'</span>';
		echo '<span>'.$row['testimony_title'].'</span>';
		echo '<span class="edit"><a href="'.$_SERVER['PHP_SELF'].'?eid='.$row['id'].'">edit</a></span>';
		echo '<span class="delete"><a href="'.$_SERVER['PHP_SELF'].'?did='.$row['id'].'">delete</a></span>';
		echo '</div>';
	}
	...
	for ($i=1;$i<=$totalPageNum;$i++){
		$classValue = $currentPageId == $i ? 'current':'noncurrent';
		echo '<span class="'.$classValue.'"><a href="'.$_SERVER['PHP_SELF'].'?pid='.$i.'">'.$i.'</a></span>';
	}
	echo '</div>';
